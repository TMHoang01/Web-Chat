...
<!--            <span>+4</span>-->
    ...
    <div class="chat-area-footer">
        ...
        <?= $this->Form->input('sendMessage',['placeholder'=>"Type something here...", 'id'=>'sendMessage' ]) ?>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#sendMessage').keypress(function(e) {
            if (e.which == 13) {
                console.log('enter send msg', $('[name="_csrfToken"]').val());
        ...
        function sendMsg(to, message){
            $.ajax({
                url: '<?= $this->Url->build(['action'=>'sendMessage']) ?>',
                type: 'POST',
                ...
                success: function(data) {
                    console.log(data);
                    $('#chat-area-main').append(data);
                    $('.chat-area-footer').scrollTop($('.chat-area-footer')[0].scrollHeight);
                }
            });
        };
    });
